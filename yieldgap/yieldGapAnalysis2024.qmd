...
# theoretical figure
read.csv("Copy of Kernza Agronomy interview  (Responses) - Sheet1.csv") ->onfarmdat  

knitr::kable(onfarmdat)

onfarmdat %>% 
  rename("site" = "farm") %>% 
  rename(yield_lbsA = yield) %>%
  filter(site == "wendy" | site == "jokela" | site == "kimber") %>%  
  mutate(type = "actual") %>%    select(-unit) -> onfarmdat2 

dat2 %>%
  select(site,yield_lbsA) %>% 
  group_by(site) %>%
  summarise(yield_lbsA = mean(yield_lbsA)) %>%
  mutate(quality = "bin run") %>%
  mutate(type = "predicted") %>%
  full_join(onfarmdat2) %>%
  filter(site == "wendy" | site == "jokela" | site == "kimber") %>%
  pivot_wider(names_from = type,values_from = yield_lbsA)  -> dat3  

knitr::kable(dat3, caption = "predicted vs actual kernza grain yields (lbs A)")   

dat3 %>%
  filter(site == "jokela" | site == "wendy") %>%
  ...
  xlim(0,700) +
  ylim(0,700) +  
  labs(caption="predicted vs actual kernza grain yields (lbs A) at bin run quality") 
```
