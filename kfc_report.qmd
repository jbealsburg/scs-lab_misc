...
```{r}
#| label: fig-lodging
#| fig-cap: "Forage lodging scores at harvest (anthesis). Yield presented for comparison. Notice some of the lower yielding plots had lower lodging as well."
...
dat %>%
  filter(activity == "harvest") %>%
  # glimpse()
  select(plot, trtIWG, yield_drymatter_kgha, lodging_score, moisture_percent) %>%
  mutate(Entry = trtIWG) %>% 
  group_by(Entry) %>% 
  summarise(`Moisture content %`= round(mean(moisture_percent),1)) %>% 
  knitr::kable(
    caption = "No differences among moisture content between varieties at harvest"
  )
```
